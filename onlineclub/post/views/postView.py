...
def home(request, club_id):
    ...
    return render(request, 'post/home.html', {'posts':posts, 'club_id':club_id})

def detail(request, club_id, id):
    post = get_object_or_404(Posts, club_id=club_id, post_id=id)
    return render(request, 'post/detail.html', {'post':post, 'club_id':club_id})

def new(request, club_id):
    return render(request, 'post/new.html', {'club_id':club_id})
