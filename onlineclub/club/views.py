...
def home(request):
    ...
    # 사이드메뉴 (학술 봉사 문화)
    # club_type_count = ClubTypes.objects.annotate(Count('club_type'))
    ...
    # 검색어 무
        ...
        side_club_type_1 = Clubs.objects.all().filter(club_type = 1).order_by('club_name')
...
def haksool(request, club_type):
    template_name = 'home.html'
    haksool_list = Clubs.objects.all().filter(club_type = club_type).order_by('club_name')
    return render(request, template_name, {'club_list': haksool_list})
